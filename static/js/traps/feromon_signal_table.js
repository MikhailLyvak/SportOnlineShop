function newUrl(order_by, slug) {
    return `/api/feromon_trap_signals_api/${slug}/?ordering=${order_by}&page=1`;
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    let nextUrl = null;
    let prevUrl = null;
    const orderFields = ['visit_time', 'added_by', 'moth_type', 'moth_amount'];
    ...
    function updateTable(url) {
      fetch(url)
        ...
        .then(data => {
          const tableBody = document.getElementById('feromon_signals_table_api').getElementsByTagName('tbody')[0];
          tableBody.innerHTML = '';
          ...
          data.results.forEach(item => {
            ...
            cell = row.insertCell();
            cell.innerText = item.moth_amount;
            cell.classList.add("text-center");
            cell = row.insertCell();
            cell.innerText = item.moth_type;
            cell.classList.add("text-center");
            cell = row.insertCell();
  ...
  document.getElementById('prev_button').addEventListener('click', function () {
    ...
    document.getElementById("moth_type").addEventListener('click', function () {
      orderBy = (orderBy === "moth_type") ? "-moth_type" : "moth_type";
      currentPage = 1;
    ...
    document.getElementById("moth_amount").addEventListener('click', function () {
      orderBy = (orderBy === "moth_amount") ? "-moth_amount" : "moth_amount";
      currentPage = 1;
  
      mlAddIcon("moth_amount")
  ...
  function animatedNumers() {
    const obj1 = document.getElementById("moth_sum_animation");
    animateValue(obj1, 0, moth_amount_sum);
