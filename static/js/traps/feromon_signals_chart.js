function fetchData(slug, period) {
    const url = `/api/feromon-chart-data/${slug}/${period}`;
...
return fetchData(trap_slug, period)
    .then(data => {
      ...
      data.forEach(entry => {
          moth_amount.push(entry.moth_amount);
          visitTime.push(entry.visit_time);
      });

      return { moth_amount, visitTime };
    })
    .catch(error => {
        console.error('Error:', error);
        throw error;
